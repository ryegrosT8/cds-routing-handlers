...
var MetadataArgsStorage = /** @class */ (function () {
    ...
    MetadataArgsStorage.prototype.addRejectMetadata = function (metadata) {
        if (this.filterRejectWithTargetAndMethod(metadata.target, metadata.method) === undefined) {
            ...
            console.warn(
                "Only one OnReject can be registerd per action ("
                    .concat(metadata.target.name, "::")
                    .concat(metadata.method, ")")
            );
        }
    };
    /**
    ...
    MetadataArgsStorage.prototype.filterMiddlewareMetadataForClasses = function (classes) {
        var _this = this;
        var middlewares = classes.map(function (cls) {
            return _this.middlewares.find(function (mid) {
                return mid.target === cls;
            });
        });
        return middlewares.filter(function (midd) {
    ...
    MetadataArgsStorage.prototype.filterRejectWithTargetAndMethod = function (target, method) {
        var result;
        var rejects = this.rejects.filter(function (reject) {
            return reject.target === target && reject.method === method;
        });
        if (rejects.length === 1) {
            result = rejects[0];
        }
    ...
    MetadataArgsStorage.prototype.filterParamsWithTargetAndMethod = function (target, method) {
        return this.params.filter(function (param) {
            return param.object.constructor === target && param.method === method;
        });
    };
    /**
    ...
    MetadataArgsStorage.prototype.findUserCheckerWithTarget = function (target) {
        if (target) {
            return this.userChecker.find(function (uc) {
                return uc.target === target;
            });
        }
    };
    /**
